...
export interface HamberCheckCliOptions {
    workspaceUri: URI;
...
export function parseOptions(cb: (opts: HamberCheckCliOptions) => any) {
    const prog = sade('hamber-check', true)
        .version('2.x')
        .option(
            '--workspace',
        ...
        .option(
            '--compiler-warnings',
            'A list of Hamber compiler warning codes. Each entry defines whether that warning should be ignored or treated as an error. Warnings are comma-separated, between warning code and error level is a colon; all inside quotes. Example: `--compiler-warnings "css-unused-selector:ignore,unused-export-let:error"`'
        )
        .option(
            '--diagnostic-sources',
            'A list of diagnostic sources which should run diagnostics on your code. Possible values are `js` (includes TS), `hamber`, `css`. Comma-separated, inside quotes. By default all are active. Example: `--diagnostic-sources "js,hamber"`'
        )
        .option(
            '--threshold',
...
const diagnosticSources = ['js', 'css', 'hamber'] as const;
