...
import { offsetAt } from 'hamber-language-server';
...
export class HumanFriendlyWriter implements Writer {
    ...
    start(workspaceDir: string) {
        ...
        if (this.isVerbose) {
            this.stream.write('\n');
            this.stream.write('====================================\n');
            this.stream.write(`Loading hamber-check in workspace: ${workspaceDir}`);
            this.stream.write('\n');
            this.stream.write('Getting Hamber diagnostics...\n');
            this.stream.write('\n');
        }
    }
