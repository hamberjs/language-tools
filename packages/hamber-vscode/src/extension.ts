...
import { setupHamberKit } from './hamberkit';
...
export function activate(context: ExtensionContext) {
    // The extension is activated on TS/JS/Hamber files because else it might be too late to configure the TS plugin:
    // If we only activate on Hamber file and the user opens a TS file first, the configuration command is issued too late.
    // We wait until there's a Hamber file open and only then start the actual language client.
    const tsPlugin = new TsPlugin(context);
    let lsApi: { getLS(): LanguageClient } | undefined;

    if (workspace.textDocuments.some((doc) => doc.languageId === 'hamber')) {
        lsApi = activateHamberLanguageServer(context);
        tsPlugin.askToEnable();
    } else {
        const onTextDocumentListener = workspace.onDidOpenTextDocument((doc) => {
            if (doc.languageId === 'hamber') {
                lsApi = activateHamberLanguageServer(context);
                tsPlugin.askToEnable();
...
export function activateHamberLanguageServer(context: ExtensionContext) {
    warnIfOldExtensionInstalled();

    const runtimeConfig = workspace.getConfiguration('hamber.language-server');
    ...
    const serverModule = require.resolve(lsPath || 'hamber-language-server/bin/server.js');
    ...
    const clientOptions: LanguageClientOptions = {
        documentSelector: [{ scheme: 'file', language: 'hamber' }],
        ...
        initializationOptions: {
            configuration: {
                hamber: workspace.getConfiguration('hamber'),
    ...
    context.subscriptions.push(
        commands.registerCommand('hamber.restartLanguageServer', async () => {
    ...
    async function restartLS(showNotification: boolean) {
        ...
        await ls.onReady();
        if (showNotification) {
            window.showInformationMessage('Hamber language server restarted.');
        }
        restartingLs = false;
    }
    ...
    let enabled = workspace
        .getConfiguration('hamber.plugin.hamber')
    ...
    languages.setLanguageConfiguration('hamber', {
        indentationRules: {
...
function addDidChangeTextDocumentListener(getLS: () => LanguageClient) {
    // Only Hamber file changes are automatically notified through the inbuilt LSP
    // because the extension says it's only responsible for Hamber files.
...
function addRenameFileListener(getLS: () => LanguageClient) {
    workspace.onDidRenameFiles(async (evt) => {
        ...
        if (
            lastPart.includes('.') &&
            !['.ts', '.js', '.json', '.hamber'].some((ending) => lastPart.endsWith(ending))
        ) {
            return;
        }
        ...
        window.withProgress(
            ...
            async () => {
                ...
                // - A Hamber file is moved/renamed
                //      -> all updates will be related to that Hamber file, do that here. The TS LS won't even notice the update
                // - A TS/JS file is moved/renamed
                //      -> all updates will be related to that TS/JS file
                //      -> let the TS LS take care of these updates in TS/JS files, do Hamber file updates here
                // - A folder with TS/JS AND Hamber files is moved/renamed
                //      -> all Hamber file updates are handled here
                //      -> all TS/JS file updates that consist of only TS/JS import updates are handled by the TS LS
                //      -> all TS/JS file updates that consist of only Hamber import updates are handled here
                ...
                edits.forEach((change) => {
                    ...
                    const containsHamberImportUpdate = change.edits.some((edit) =>
                        edit.newText.endsWith('.hamber')
                    );
                    if (isTsOrJsFile && !containsHamberImportUpdate) {
                        return;
                    }
                    ...
                    change.edits.forEach((edit) => {
                        if (
                            ...
                            // TS plugin disabled -> let TS/JS path updates be handled by the TS LS, Hamber here
                            return;
                        }
...
function addCompilePreviewCommand(getLS: () => LanguageClient, context: ExtensionContext) {
    ...
    context.subscriptions.push(
        commands.registerTextEditorCommand('hamber.showCompiledCodeToSide', async (editor) => {
            if (editor?.document?.languageId !== 'hamber') {
                return;
            }

            const uri = editor.document.uri;
            const hamberUri = CompiledCodeContentProvider.toHamberSchemeUri(uri);
            window.withProgress(
                { location: ProgressLocation.Window, title: 'Compiling..' },
                async () => {
                    return await window.showTextDocument(hamberUri, {
                        preview: true,
...
function addExtracComponentCommand(getLS: () => LanguageClient, context: ExtensionContext) {
    context.subscriptions.push(
        commands.registerTextEditorCommand('hamber.extractComponent', async (editor) => {
            if (editor?.document?.languageId !== 'hamber') {
                return;
            }
            ...
            window.showInputBox(options).then(async (filePath) => {
                ...
                getLS().sendRequest(ExecuteCommandRequest.type, {
                    command: 'extract_to_hamber_component',
...
function createLanguageServer(serverOptions: ServerOptions, clientOptions: LanguageClientOptions) {
    return new LanguageClient('hamber', 'Hamber', serverOptions, clientOptions);
}

function warnIfOldExtensionInstalled() {
    if (extensions.getExtension('JamesBirtles.hamber-vscode')) {
        window.showWarningMessage(
            'It seems you have the old and deprecated extension named "Hamber" installed. Please remove it. ' +
...
async function noteOfNewTransformation() {
    ...
    const response = await window.showInformationMessage(
        'The Hamber for VS Code extension comes with a new transformation for improved intellisense. ' +
            'It is enabled by default now. If you notice bugs, please report them. ' +
            'You can switch to the old transformation setting "hamber.plugin.hamber.useNewTransformation" to "false".',
        ...answers
    );
