...
async function handleMultiple(uri: Uri | undefined) {
    ...
    const opts = [
        ...
        // const iconName = resource.type === FileType.PAGE ? 'hamber' : isTs ? 'typescript' : 'javascript';
        const extension = resource.type === FileType.PAGE ? 'hamber' : scriptExtension;
        return {
            // TODO: maybe add icons (ts,js,hamber - but it doesn´t work like this)
...
async function generate(config: GenerateConfig) {
    await window.withProgress(
        { location: ProgressLocation.Window, title: 'Creating HamberKit files...' },
        async () => {
