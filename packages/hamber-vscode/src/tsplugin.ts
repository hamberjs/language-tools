...
export class TsPlugin {
    ...
    static isEnabled(): boolean {
        return workspace.getConfiguration('hamber').get<boolean>('enable-ts-plugin') ?? false;
    }
    ...
    private async toggleTsPlugin(enable: boolean) {
        ...
        commands.executeCommand('_typescript.configurePlugin', 'typescript-hamber-plugin', {
            enable
        });
    }
    ...
    async askToEnable() {
        ...
        const response = await window.showInformationMessage(
            'The Hamber for VS Code extension now contains a TypeScript plugin. ' +
                'Enabling it will provide intellisense for Hamber files from TS/JS files. ' +
        ...
        if (response === answers[2]) {
            workspace.getConfiguration('hamber').update('enable-ts-plugin', true, true);
        } else if (response === answers[1]) {
            workspace.getConfiguration('hamber').update('ask-to-enable-ts-plugin', false, true);
        }
    }
}
