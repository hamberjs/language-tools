...
import { isNoTextSpanInGeneratedCode, isHamberFilePath } from './utils';

export class HamberSnapshot {
    ...
    constructor(
        ...
        private hamberCode: string,
        private mapper: SourceMapper,
        private logger: Logger,
        public readonly isTsFile: boolean
    ) {}

    update(hamberCode: string, mapper: SourceMapper) {
        this.hamberCode = hamberCode;
        this.mapper = mapper;
    ...
    setAndPatchScriptInfo(scriptInfo: ts.server.ScriptInfo) {
        ...
        //         start,
        //         length: (lineOffset[line] || this.hamberCode.length) - start
        //     };
    ...
    positionAt(offset: number): ts.LineAndCharacter {
        offset = this.clamp(offset, 0, this.hamberCode.length);
    ...
    private log(...args: any[]) {
        this.logger.log('HamberSnapshot:', this.fileName, '-', ...args);
    }

    private debug(...args: any[]) {
        this.logger.debug('HamberSnapshot:', this.fileName, '-', ...args);
    }
...
export class HamberSnapshotManager {
    private snapshots = new Map<string, HamberSnapshot>();

    constructor(
        private typescript: typeof ts,
        private projectService: ts.server.ProjectService,
        private hamberOptions: { namespace: string },
        private logger: Logger,
    ...
    create(fileName: string): HamberSnapshot | undefined {
        ...
        if (!snapshot) {
            this.logger.log(
                'Hamber snapshot was not found after trying to load script snapshot for',
                fileName
            );
    ...
    private patchProjectServiceReadFile() {
        ...
        this.projectService.host.readFile = (path: string, encoding?: string | undefined) => {
            ...
            if (normalizedPath.endsWith('node_modules/hamber/types/runtime/ambient.d.ts')) {
                return '';
            } else if (normalizedPath.endsWith('hamber2tsx/hamber-shims.d.ts')) {
                let originalText = readFile(path) || '';
                if (!originalText.includes('// -- start hamber-ls-remove --')) {
                    return originalText; // uses an older version of hamber2tsx
                }
                originalText =
                ...
                return originalText;
            } else if (isHamberFilePath(path)) {
                this.logger.debug('Read Hamber file:', path);
                const hamberCode = readFile(path) || '';
                try {
                    const isTsFile = true; // TODO check file contents? TS might be okay with importing ts into js.
                    const result = hamber2tsx(hamberCode, {
                        filename: path.split('/').pop(),
                        isTsFile,
                        mode: 'ts', // useNewTransformation
                        typingsNamespace: this.hamberOptions.namespace
                    });
                    const existingSnapshot = this.snapshots.get(path);
                    if (existingSnapshot) {
                        existingSnapshot.update(hamberCode, new SourceMapper(result.map.mappings));
                    } else {
                        this.snapshots.set(
                            path,
                            new HamberSnapshot(
                    ...
                    this.logger.log('Successfully read Hamber file contents of', path);
                    return result.code;
                } catch (e) {
                    this.logger.log('Error loading Hamber file:', path, ' Using fallback.');
                    this.logger.debug('Error:', e);
                    // Return something either way, else "X is not a module" errors will appear
                    // in the TS files that use this file.
                    return 'export default class extends Hamber2TsxComponent<any,any,any> {}';
                }
            } else {
                return readFile(path, encoding);
            }
