...
/**
 * Handles HTML elements as well as hamber:options, hamber:head, hamber:window, hamber:body, hamber:element
 *
 ...
 * <div class={foo} />
 * // after
 * { const $$_div = __hamberts_2_createElement("div", {"class": foo,}); }
 * ```
 */
export class Element {
     ...
     * @param node The Hamber AST node that represents this element
     * @param typingsNamespace Determines which namespace to use for the createElement function
     * @param parent The Hamber AST parent node
     */
    constructor(
        ...
        this.tagName = this.node.name === 'hamber:body' ? 'body' : this.node.name;
        ...
        switch (this.node.name) {
            ...
            // is valid on the special hamber elements,
            // we still also handle them here and let the Hamber parser handle invalid
            // cases. For us it doesn't make a difference to a normal HTML element.
            case 'hamber:options':
            case 'hamber:head':
            case 'hamber:window':
            case 'hamber:body':
            case 'hamber:fragment': {
                // remove the colon: hamber:xxx -> hamberxxx
                const nodeName = `hamber${this.node.name.substring(7)}`;
                this._name = '$$_' + nodeName + this.computeDepth();
                break;
            }
            case 'hamber:element': {
                this._name = '$$_hamberelement' + this.computeDepth();
                break;
            }
            case 'slot': {
    ...
    addAction(attr: BaseDirective) {
        ...
        this.actionsTransformation.push(
            `const ${id} = __hamberts_2_ensureAction(`,
    ...
    private getStartTransformation(): TransformationArray {
        ...
        const addActions = () => {
            if (this.actionIdentifiers.length) {
                return `, __hamberts_2_union(${this.actionIdentifiers.join(',')})`;
            } else {
                return '';
            }
        };
