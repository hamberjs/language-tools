 ...
 *   `{ const $$_each = __hamberts_2_ensureArray(items); for (const items of $$_each) { let i = 0;key;`.
 *   Transform it this way because `{#each items as items}` is valid Hamber code, but the transformation
...
export function handleEach(str: MagicString, eachBlock: BaseNode): void {
    ...
    if (arrayAndItemVarTheSame) {
        transforms = [
            `{ const $$_each = __hamberts_2_ensureArray(${containsComma ? '(' : ''}`,
        ...
        transforms = [
            'for(let ',
            [eachBlock.context.start, eachBlock.context.end],
            ` of __hamberts_2_ensureArray(${containsComma ? '(' : ''}`,
            [eachBlock.expression.start, eachBlock.expression.end],
            `${containsComma ? ')' : ''})){`
        ];
    }
