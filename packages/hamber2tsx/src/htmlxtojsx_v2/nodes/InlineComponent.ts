...
/**
 * Handles Hamber components as well as hamber:self and hamber:component
 *
 ...
 * <Comp prop={foo} />
 * // after
 * { const $$_Comp = new Comp({ target: __hamberts_2_any(), props: {"prop": foo,}}); }
 * ```
 */
export class InlineComponent {
    ...
    constructor(private str: MagicString, private node: BaseNode, public parent?: any) {
        ...
        if (this.node.name === 'hamber:self') {
            ...
            this._name = '$$_hamberself' + this.computeDepth();
            this.startTransformation.push('{ __hamberts_2_createComponentAny({');
            this.addNameConstDeclaration = () =>
                (this.startTransformation[0] = `{ const ${this._name} = __hamberts_2_createComponentAny({`);
            this.startEndTransformation.push('});');
        } else {
            const isHamberComponentTag = this.node.name === 'hamber:component';
            // We don't know if the thing we use to create the Hamber component with
            // is actually a proper Hamber component, which would lead to errors
            ...
            // will be on the __hamberts_2_ensureComponent part, giving a more helpful message
            this._name = '$$_' + sanitizePropName(this.node.name) + this.computeDepth();
            const constructorName = this._name + 'C';
            const nodeNameStart = isHamberComponentTag
                ? this.node.expression.start
                : this.str.original.indexOf(this.node.name, this.node.start);
            const nodeNameEnd = isHamberComponentTag
                ? this.node.expression.end
                : nodeNameStart + this.node.name.length;
            this.startTransformation.push(
                `{ const ${constructorName} = __hamberts_2_ensureComponent(`,
                [nodeNameStart, nodeNameEnd],
                `); new ${constructorName}({ target: __hamberts_2_any(), props: {`
            );
            this.addNameConstDeclaration = () =>
                (this.startTransformation[2] = `); const ${this._name} = new ${constructorName}({ target: __hamberts_2_any(), props: {`);
            this.startEndTransformation.push('}});');
        }
    }
    ...
    performTransformation(): void {
        ...
        if (this.isSelfclosing) {
            ...
            if (!this.node.name.startsWith('hamber:')) {
