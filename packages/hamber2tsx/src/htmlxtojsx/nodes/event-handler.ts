 ...
 * - For Hamber components/special elements: ---> {__hamberts_1_instanceOf(..ComponentType..).$on("xxx", yyy)}
 */
export function handleEventHandler(
    ...
    if (
        ...
        if (attr.expression) {
            const on = 'on';
            //for handler assignment, we change it to call to our __hamberts_1_ensureFunction
