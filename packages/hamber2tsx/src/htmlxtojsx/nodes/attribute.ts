...
/**
 * List taken from `hamber-jsx.d.ts` by searching for all attributes of type number
 */
...
export function handleAttribute(
    ...
    const slotName = shouldApplySlotCheck
        ...
        : undefined;
    const ensureSlotStr = `__hamberts_ensureSlot("${slotName}","${attr.name}",`;
    ...
    if (parent.type == 'Element') {
        const ramberLinkActions = ['ramber:prefetch', 'ramber:noscroll'];
        const hamberkitLinkActions = [
            'hamberkit:prefetch',
            'hamberkit:noscroll',
            'hamberkit:reload'
        ];
        ...
        if (
            ...
            ramberLinkActions.includes(attr.name) ||
            hamberkitLinkActions.includes(attr.name)
        ) {
    ...
    if (parent.type === 'InlineComponent' && attr.name.startsWith('--') && attr.value !== true) {
        str.prependRight(attr.start, '{...__hamberts_1_cssProp({"');
