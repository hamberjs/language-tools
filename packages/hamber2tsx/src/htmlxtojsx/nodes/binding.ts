...
export function handleBinding(
    ...
    if (attr.name == 'group' && el.name == 'input') {
        ...
        str.appendLeft(attr.expression.start, '{...__hamberts_1_empty(');
    ...
    if (attr.name === 'this' && supportsBindThis.includes(el.type)) {
        ...
        // Some notes:
        // - If the component unmounts (it's inside an if block, or hamber:component this={null},
        ...
        if (thisType) {
            str.overwrite(attr.start, attr.expression.start, '{...__hamberts_1_empty(');
    ...
    if (oneWayBindingAttributes.has(attr.name) && el.type === 'Element') {
        ...
        if (isShortHandAttribute(attr)) {
            ...
            str.appendLeft(
                attr.end,
                `=__hamberts_1_instanceOf(${oneWayBindingAttributes.get(attr.name)}).${attr.name})}`
            );
        } else {
            // eslint-disable-next-line max-len
            str.overwrite(
