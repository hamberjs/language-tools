...
/**
 * style:xx         --->  __hamberts_1_ensureType(String, Number, xx);
 * style:xx={yy}    --->  __hamberts_1_ensureType(String, Number, yy);
 * style:xx="yy"    --->  __hamberts_1_ensureType(String, Number, "yy");
 * style:xx="a{b}"  --->  __hamberts_1_ensureType(String, Number, `a${b}`);
 */
export function handleStyleDirective(str: MagicString, style: StyleDirective): void {
    ...
    if (style.value === true || style.value.length === 0) {
        str.overwrite(
            ...
            '{...__hamberts_1_ensureType(String, Number, '
        );
    ...
    if (styleVal.type === 'Text') {
        str.overwrite(style.start, styleVal.start, '{...__hamberts_1_ensureType(String, Number, "');
