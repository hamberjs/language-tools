 ...
 * @param thisValue If node is hamber:component, you may pass the value
 *                  of this={..} to use that instead of the more general componentType
 */
export function getTypeForComponent(node: Node): string {
    if (node.name === 'hamber:component' || node.name === 'hamber:self') {
        return '__hamberts_1_componentType()';
    } else {
        return node.name;
    }
}
...
export function getInstanceTypeSimple(node: Node, str: MagicString): string | undefined {
    const instanceOf = (str: string) => `__hamberts_1_instanceOf(${str})`;

    switch (node.type) {
        case 'InlineComponent':
            if (node.name === 'hamber:component' && node.expression) {
                const thisVal = str.original.substring(node.expression.start, node.expression.end);
                return `new (${thisVal})({target: __hamberts_1_any(''), props: __hamberts_1_any('')})`;
            } else if (node.name === 'hamber:component' || node.name === 'hamber:self') {
                return instanceOf('__hamberts_1_componentType()');
            } else {
                return `new ${node.name}({target: __hamberts_1_any(''), props: __hamberts_1_any('')})`;
            }
        case 'Element':
            return instanceOf(`__hamberts_1_ctorOf(__hamberts_1_mapElementTag('${node.name}'))`);
        case 'Body':
...
export function getInstanceType(
    ...
    if (node.name === 'hamber:component' || node.name === 'hamber:self') {
        return '__hamberts_1_instanceOf(__hamberts_1_componentType())';
    }
    ...
    return surroundWithIgnoreComments(
        `new ${node.name}({target: __hamberts_1_any(''), props: {${propsStr}}})`
    );
}
...
export function getInstanceTypeForDefaultSlot(
    ...
    if (node.name === 'hamber:component' || node.name === 'hamber:self') {
        return {
            str: '__hamberts_1_instanceOf(__hamberts_1_componentType())',
            shadowedProps: []
        };
    }
 ...
 * Usually it's there because of the preprocessing we do before we let Hamber parse the template.
 */
