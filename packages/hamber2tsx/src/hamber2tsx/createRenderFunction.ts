...
export function createRenderFunction({
    ...
    if (uses$$props) {
        propsDecl += ' let $$props = __hamberts_1_allPropsType();';
    }
    if (uses$$restProps) {
        propsDecl += ' let $$restProps = __hamberts_1_restPropsType();';
    }

    if (uses$$slots) {
        propsDecl +=
            ' let $$slots = __hamberts_1_slotsType({' +
            ...
            ? '\n' +
              surroundWithIgnoreComments(
                  useNewTransformation
                      ? ';const __hamberts_createSlot = __hamberts_2_createCreateSlot' +
                            (uses$$SlotsInterface ? '<$$Slots>' : '') +
                            '();'
                      : ';const __hamberts_ensureSlot = __hamberts_1_createEnsureSlot' +
                            (uses$$SlotsInterface ? '<$$Slots>' : '') +
                            '();'
              )
            : '';
