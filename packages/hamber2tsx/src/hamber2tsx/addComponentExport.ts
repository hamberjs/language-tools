 ...
 */
export const COMPONENT_SUFFIX = '__HamberComponent_';
...
function addGenericsComponentExport({
    ...
    function returnType(forPart: string) {
        return `ReturnType<__hamberts_Render${genericsRef}['${forPart}']>`;
    }

    let statement = `
class __hamberts_Render${genericsDef} {
    props() {
    ...
    if (mode === 'dts') {
        statement +=
            ...
            `\n${doc}export default class${
                className ? ` ${className}` : ''
            }${genericsDef} extends HamberComponentTyped<${className}Props${genericsRef}, ${className}Events${genericsRef}, ${className}Slots${genericsRef}> {` + // eslint-disable-line max-len
        ...
        statement +=
            `\n\n${doc}export default class${
                className ? ` ${className}` : ''
            }${genericsDef} extends Hamber2TsxComponent<${returnType('props')}, ${returnType(
                'events'
            )}, ${returnType('slots')}> {` +
            exportedNames.createClassGetters() +
            (usesAccessors ? exportedNames.createClassAccessors() : '') +
...
function addSimpleComponentExport({
    ...
    if (mode === 'dts' && isTsFile) {
        statement =
            ...
            `\n${doc}export default class${
                className ? ` ${className}` : ''
            } extends HamberComponentTyped<${className}Props, ${className}Events, ${className}Slots> {` + // eslint-disable-line max-len
            exportedNames.createClassGetters() +
            (usesAccessors ? exportedNames.createClassAccessors() : '') +
            '\n}';
        ...
        statement =
            ...
            `\n${doc}export default class${
                className ? ` ${className}` : ''
            } extends __hamberts_1_createHamberComponentTyped(${propDef}) {` +
            exportedNames.createClassGetters() +
            (usesAccessors ? exportedNames.createClassAccessors() : '') +
            '\n}';
    } else {
        statement =
            `\n\n${doc}export default class${
                className ? ` ${className}` : ''
            } extends __hamberts_1_createHamber2TsxComponent(${propDef}) {` +
            exportedNames.createClassGetters() +
            (usesAccessors ? exportedNames.createClassAccessors() : '') +
            '\n}';
    }
...
function events(strictEvents: boolean, renderStr: string) {
    return strictEvents ? renderStr : `__hamberts_1_with_any_event(${renderStr})`;
}
...
function props(
    ...
    if (isTsFile) {
        return canHaveAnyProp ? `__hamberts_1_with_any(${renderStr})` : renderStr;
    } else {
        const optionalProps = exportedNames.createOptionalPropsArray();
        const partial = `__hamberts_1_partial${canHaveAnyProp ? '_with_any' : ''}`;
...
/**
 * Returns a Hamber-compatible component name from a filename. Hamber
 * components must use capitalized tags, so we try to transform the filename.
 *
 * https://hamberjs.web.app/docs#Tags
 */
