...
export class ComponentEvents {
     ...
     * of the return object of the `hamber2tsx` function.
     */
    createAPI() {
    ...
    checkIfDeclarationInstantiatedEventDispatcher(node: ts.VariableDeclaration) {
        ...
        if (!dispatcherTyping) {
            this.str.prependLeft(
                ...
                '<__hamberts_1_CustomEvents<$$Events>>'
            );
        }
    }
            ...
            [
                ...this.eventDispatchers
                    .map(
                        (dispatcher) =>
                            dispatcher.typing &&
                            `...__hamberts_1_toEventTypings<${dispatcher.typing}>()`
                    )
                    .filter((str) => !!str),
                ...this.eventHandler.bubbledEventsAsStrings(),
                ...[...this.dispatchedEvents.keys()].map((e) => `'${e}': __hamberts_1_customEvent`)
            ].join(', ') +
            '}'
        );
    }
...
function checkIfImportIsEventDispatcher(node: ts.ImportDeclaration): string | undefined {
    if (ts.isStringLiteral(node.moduleSpecifier) && node.moduleSpecifier.text !== 'hamber') {
        return;
    }
