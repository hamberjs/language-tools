...
export class EventHandler {
    ...
    private handleEventHandlerBubble(parent: Node, eventName: string): void {
        const componentEventDef = `__hamberts_1_instanceOf(${parent.name})`;
        const exp = `__hamberts_1_bubbleEventDef(${componentEventDef}.$$events_def, '${eventName}')`;
...
function getEventDefExpressionForNonCompoent(eventName: string, ele: Node) {
    switch (ele.type) {
        case 'Element':
            return `__hamberts_1_mapElementEvent('${eventName}')`;
        case 'Body':
            return `__hamberts_1_mapBodyEvent('${eventName}')`;
        case 'Window':
            return `__hamberts_1_mapWindowEvent('${eventName}')`;
        default:
            break;
    }
}
...
function eventMapEntryToString([eventName, expression]: [string, string | string[]]) {
    return `'${eventName}':${
        Array.isArray(expression) ? `__hamberts_1_unionType(${expression.join(',')})` : expression
    }`;
}
