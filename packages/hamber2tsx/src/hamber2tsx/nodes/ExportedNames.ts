...
export class ExportedNames {
    ...
    /**
     * Appends `prop = __hamberts_1_any(prop)`  to given declaration in order to
        ...
        const handleTypeAssertion = (declaration: ts.VariableDeclaration) => {
            ...
            if (
                ...
                preprendStr(
                    ...
                    surroundWithIgnoreComments(`;${name} = __hamberts_1_any(${name});`)
                );
            }
        };
    ...
    createPropsStr(isTsFile: boolean): string {
        ...
        if (this.uses$$Props) {
            ...
            // __hamberts_1_ensureRightProps needs to be declared in a way that doesn't affect the type result of props
            return (
                '{...__hamberts_1_ensureRightProps<{' +
                this.createReturnElementsType(lets).join(',') +
                '}>(__hamberts_1_any("") as $$Props), ' +
                '...__hamberts_1_ensureRightProps<Partial<$$Props>>({' +
