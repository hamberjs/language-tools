...
import {
    ...
    isObjectValue,
    getSlotName
} from '../../utils/hamberAst';
import TemplateScope from './TemplateScope';
import { HamberIdentifier, WithName } from '../../interfaces';
...
export class SlotHandler {
    ...
    resolve(identifierDef: HamberIdentifier, initExpression: Node, scope: TemplateScope) {
     ...
     *
     * Example: {#each items as item} ---> __hamberts_1_unwrapArr(items)
     */
    private getResolveExpressionStr(
        identifierDef: HamberIdentifier,
        scope: TemplateScope,
        initExpression: Node
    ) {
        ...
        // initExpression's scope should the parent scope of identifier scope
            ...
            return `__hamberts_1_unwrapPromiseLike(${resolvedExpression})`;
        } else if (owner?.type === 'EachBlock') {
            const resolvedExpression = this.resolveExpression(initExpression, scope.parent);

            return `__hamberts_1_unwrapArr(${resolvedExpression})`;
        }
        return null;
    }
...
function getSingleSlotDef(componentNode: Node, slotName: string) {
    ...
    return `__hamberts_1_instanceOf(${componentType}).$$slot_def['${slotName}']`;
}
