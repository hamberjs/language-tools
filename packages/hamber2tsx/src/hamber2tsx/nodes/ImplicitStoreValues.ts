 ...
 * were used as stores are appended with `let $xx = __hamberts_1_store_get(xx)` to create the store variables.
 */
export class ImplicitStoreValues {
    ...
    private createStoreDeclaration(storeName: string): string {
        return `;let $${storeName} = __hamberts_1_store_get(${storeName});`;
    }
}
