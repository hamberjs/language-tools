...
type TemplateProcessResult = {
    /**
     * The HTML part of the Hamber AST.
     */
...
function processHamberTemplate(
    str: MagicString,
    options?: {
    ...
    const handleHamberOptions = (node: Node) => {
...
export function hamber2tsx(
    ...
    const str = new MagicString(hamber);
    // process the htmlx as a hamber template
    let {
        htmlAst,
        moduleScriptTag,
        scriptTag,
        ...
        resolvedStores,
        usesAccessors
    } = processHamberTemplate(str, options);
    ...
    if (options.mode === 'dts') {
        ...
        str.prepend(
            'import { HamberComponentTyped } from "hamber"\n' +
                (options?.isTsFile
                    ? ''
                    : // Not part of hamber-shims.d.ts because it would throw type errors as this function assumes
                      // the presence of a HamberComponentTyped import
                      `
declare function __hamberts_1_createHamberComponentTyped<Props, Events, Slots>(
    render: {props: Props, events: Events, slots: Slots }
): HamberComponentConstructor<HamberComponentTyped<Props, Events, Slots>,Hamber2TsxComponentConstructorParameters<Props>>;
`) +
                '\n'
        );
        ...
        str.prepend('///<reference types="hamber" />\n');
        return {
            code: str.toString(),
