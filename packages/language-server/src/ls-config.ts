...
const defaultLSConfig: LSConfig = {
    ...
    hamber: {
        ...
        format: {
            enable: true,
            config: {
                hamberSortOrder: 'options-scripts-markup-styles',
                hamberStrictMode: false,
                hamberAllowShorthand: true,
                hamberBracketNewLine: true,
                hamberIndentScriptAndStyle: true,
                printWidth: 80,
                singleQuote: false
            }
        },
 ...
 * Should be kept in sync with infos in `packages/hamber-vscode/package.json`.
 */
export interface LSConfig {
    typescript: LSTypescriptConfig;
    css: LSCSSConfig;
    html: LSHTMLConfig;
    hamber: LSHamberConfig;
}
...
export interface LSHamberConfig {
    enable: boolean;
...
export class LSConfigManager {
    ...
    update(config: DeepPartial<LSConfig>): void {
        ...
        if (config.hamber?.compilerWarnings) {
            this.config.hamber.compilerWarnings = config.hamber.compilerWarnings;
        }
     ...
     * @param key a string which is a path. Example: 'hamber.diagnostics.enable'.
     */
     ...
     * - Else the Hamber extension's Prettier config is used as a starting point,
    ...
    async getFormatCodeSettingsForFile(
        ...
        return {
            ...
            baseIndentSize: prettierConfig.hamberIndentScriptAndStyle === false ? 0 : indentSize,
            indentSize,
