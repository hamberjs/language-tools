...
export type HamberCheckDiagnosticSource = 'js' | 'css' | 'hamber';

export interface HamberCheckOptions {
    compilerWarnings?: Record<string, 'ignore' | 'error'>;
    diagnosticSources?: HamberCheckDiagnosticSource[];
    /**
     * Path has to be absolute
     */
    tsconfig?: string;
    /**
     * Whether or not to use the new transformation of hamber2tsx
     */
 ...
 * for hamber-check, without the overhead of the lsp.
 */
export class HamberCheck {
    ...
    constructor(workspacePath: string, private options: HamberCheckOptions = {}) {
        Logger.setLogErrorsOnly(true);
        this.initialize(workspacePath, options);
    }

    private async initialize(workspacePath: string, options: HamberCheckOptions) {
        ...
        if (shouldRegister('hamber')) {
            this.pluginHost.register(new HamberPlugin(this.configManager));
        }
        ...
        function shouldRegister(source: HamberCheckDiagnosticSource) {
