...
import { PreprocessorGroup } from 'hamber/types/compiler/preprocess/types';
import { importHamberPreprocess } from '../../importPackage';
...
export type InternalPreprocessorGroup = PreprocessorGroup & {
    /**
     * hamber-preprocess has this since 4.x
     */
    defaultLanguages?: {
        markup?: string;
        script?: string;
...
/**
 * Loads hamber.config.{js,cjs,mjs} files. Provides both a synchronous and asynchronous
...
export class ConfigLoader {
    private configFiles = new Map<string, HamberConfig>();
    private configFilesAsync = new Map<string, Promise<HamberConfig>>();
    ...
    /**
     * Tries to load all `hamber.config.js` files below given directory
    ...
    async loadConfigs(directory: string): Promise<void> {
        ...
        try {
            const pathResults = this.globSync('**/hamber.config.{js,cjs,mjs}', {
                cwd: directory,
                ignore: ['**/node_modules/**']
            });
    ...
    private addFallbackConfig(directory: string) {
        ...
        const path = this.path.join(directory, 'hamber.config.js');
    ...
    private searchConfigPathUpwards(path: string) {
        ...
        while (currentDir !== nextDir) {
            const tryFindConfigPath = (ending: string) => {
                const path = this.path.join(currentDir, `hamber.config.${ending}`);
     ...
     * @param file
     */
    getConfig(file: string): HamberConfig | undefined {
     ...
     */
    async awaitConfig(file: string): Promise<HamberConfig | undefined> {
    ...
    private tryGetConfig(file: string, fromDirectory: string, configFileEnding: string) {
        const path = this.path.join(fromDirectory, `hamber.config.${configFileEnding}`);
    ...
    private useFallbackPreprocessor(path: string, foundConfig: boolean): HamberConfig {
        Logger.log(
            (foundConfig
                ? 'Found hamber.config.js but there was an error loading it. '
                : 'No hamber.config.js found. ') +
                'Using https://github.com/hamberjs/hamber-preprocess as fallback'
        );
        const hamberPreprocess = importHamberPreprocess(path);
        return {
