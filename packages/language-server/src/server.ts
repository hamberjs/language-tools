...
export function startServer(options?: LSOptions) {
    ...
    let hamberPlugin: HamberPlugin = undefined as any;
    ...
    connection.onInitialize((evt) => {
        ...
        Logger.setDebug(
            (evt.initializationOptions?.configuration?.hamber ||
        ...
        configManager.update(
            evt.initializationOptions?.configuration?.hamber?.plugin ||
        ...
        pluginHost.register((hamberPlugin = new HamberPlugin(configManager)));
    ...
    function notifyTsServiceExceedSizeLimit() {
        connection?.sendNotification(ShowMessageNotification.type, {
            message:
                'Hamber language server detected a large amount of JS/Hamber files. ' +
                'To enable project-wide JavaScript/TypeScript language features for Hamber files, ' +
    ...
    connection.onDidChangeConfiguration(({ settings }) => {
        configManager.update(settings.hamber?.plugin);
        ...
        Logger.setDebug(settings.hamber?.['language-server']?.debug);
    });
    ...
    connection.onRequest('$/getCompiledCode', async (uri: DocumentUri) => {
        ...
        if (doc) {
            const compiled = await hamberPlugin.getCompiledResult(doc);
            if (compiled) {
