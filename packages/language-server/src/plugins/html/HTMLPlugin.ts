...
import { hamberHtmlDataProvider } from './dataProvider';
import {
    HoverProvider,
    CompletionsProvider,
    RenameProvider,
    ...
    implements HoverProvider, CompletionsProvider, RenameProvider, LinkedEditingRangesProvider
{
    ...
    private lang = getLanguageService({
        customDataProviders: [hamberHtmlDataProvider],
    ...
    async getCompletions(document: Document, position: Position): Promise<CompletionList | null> {
        ...
        const hamberStrictMode = prettierConfig?.hamberStrictMode;
        items.forEach((item) => {
            const startQuote = hamberStrictMode ? '"{' : '{';
            const endQuote = hamberStrictMode ? '}"' : '}';
            if (!item.textEdit) {
                return;
            }
