...
const hamberEvents = [
    ...htmlData.globalAttributes!.map(mapToHamberEvent),
    {
        name: 'on:introstart',
...
const hamberAttributes: IAttributeData[] = [
    {
        name: 'bind:innerHTML',
...
const hamberTags: ITagData[] = [
    ...
    },
    {
        name: 'hamber:body',
        description:
            "As with <hamber:window>, this element allows you to add listeners to events on document.body, such as mouseenter and mouseleave which don't fire on window.",
        attributes: []
    },
    {
        name: 'hamber:head',
        ...
        attributes: [
            {
                ...
                values: [
                    ...
                    },
                    {
                        name: '{false}',
                        description:
                            'The default. Hamber will be more conservative about whether or not mutable objects have changed'
                    }
                ]
            },
            {
...
const addAttributes: Record<string, IAttributeData[]> = {
    ...
    a: [
        {
            name: 'hamberkit:noscroll',
            description:
                'HamberKit-specific attribute. Will prevent scrolling after the link is clicked.',
            valueSet: 'v'
        },
        {
            name: 'hamberkit:prefetch',
            description:
                "HamberKit-specific attribute. Will cause HamberKit to run the page's load function as soon as the user hovers over the link (on a desktop) or touches it (on mobile), rather than waiting for the click event to trigger navigation.",
            valueSet: 'v'
        },
        {
            name: 'hamberkit:reload',
            description:
                'HamberKit-specific attribute. Will cause HamberKit to do a normal browser navigation which results in a full page reload.',
            valueSet: 'v'
        }
    ],
    details: [
...
const html5Tags = htmlData.tags!.map((tag) => {
    let attributes = tag.attributes.map(mapToHamberEvent);
...
export const hamberHtmlDataProvider = newHTMLDataProvider('hamber-builtin', {
    version: 1,
    globalAttributes: [...htmlData.globalAttributes!, ...hamberEvents, ...hamberAttributes],
    tags: [...html5Tags, ...hamberTags],
    valueSets: htmlData.valueSets
});

function mapToHamberEvent(attr: IAttributeData) {
    return {
        ...attr,
