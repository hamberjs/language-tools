import { Warning } from 'hamber/types/compiler/interfaces';
...
import { HamberDocument, TranspileErrorSource } from '../HamberDocument';

/**
 * Returns diagnostics from the hamber compiler.
...
export async function getDiagnostics(
    ...
    const config = await hamberDoc.config;
    if (config?.loadConfigError) {
        return getConfigLoadErrorDiagnostics(config.loadConfigError);
    }

    try {
        return await tryGetDiagnostics(document, hamberDoc, settings);
    } catch (error) {
...
/**
 * Try to transpile and compile the hamber file and return diagnostics.
 */
async function tryGetDiagnostics(
    ...
    try {
        const res = await hamberDoc.getCompiled();
...
async function createParserErrorDiagnostic(error: any, document: Document) {
    ...
    if (diagnostic.message.includes('expected')) {
        ...
        if (isInStyle || isInScript) {
            ...
            if (isInScript) {
                diagnostic.message +=
                    '\nIf you use typescript with `hamber-preprocess`, did you add `lang="ts"` to your `script` tag? ';
            } else {
                diagnostic.message +=
                    '\nIf you use less/SCSS with `hamber-preprocess`, did you add `lang="scss"`/`lang="less"` to your `style` tag? ' +
                    scssNodeRuntimeHint;
            }
            diagnostic.message +=
                '\nDid you setup a `hamber.config.js`? ' +
                '\nSee https://github.com/hamberjs/language-tools/tree/master/docs#using-with-preprocessors for more info.';
        }
    }
...
function getConfigLoadErrorDiagnostics(error: any): Diagnostic[] {
    return [
        {
            message: 'Error in hamber.config.js\n\n' + error,
...
function getStyleErrorDiagnostics(error: any, document: Document): Diagnostic[] {
    // Error could be from another file that was mixed into the Hamber file as part of preprocessing.
    ...
    function getStyleErrorMessage() {
        if (isHamberPreprocessCannotFindModulesError(error)) {
 ...
 * A warning about a broken hamber.configs.js/preprocessor setup should be added then.
 */
function isHamberPreprocessCannotFindModulesError(error: any): error is Error {
...
const scssNodeRuntimeHint =
    'If you use SCSS, it may be necessary to add the path to your NODE runtime to the setting `hamber.language-server.runtime`, or use `sass` instead of `node-sass`. ';
