...
import { documentation, HamberTag, getLatestOpeningTag } from './HamberTags';
...
/**
 * Get hover information for special hamber tags within moustache tags.
 */
export function getHoverInfo(
    ...
    const offset = hamberDoc.offsetAt(position);

    const isInStyleOrScript =
        isInTag(position, hamberDoc.style) ||
        isInTag(position, hamberDoc.script) ||
        isInTag(position, hamberDoc.moduleScript);

    const offsetStart = Math.max(offset - 10, 0);
    const charactersAroundOffset = hamberDoc
        .getText()
        // use last 10 and next 10 characters, should cover 99% of all cases
        .substr(offsetStart, 20);
    const isHamberTag = tagRegexp.test(charactersAroundOffset);
    ...
    if (isHamberTag) {
        return getTagHoverInfoAtOffset(hamberDoc, offsetStart, charactersAroundOffset, offset);
    }
    ...
    const attributeOffset = hamberDoc.getText().lastIndexOf(attributeContext.name, offset);
...
function getTagHoverInfoAtOffset(
    ...
    const tag = getTagAtOffset(hamberDoc, charactersOffset, charactersAroundOffset, offset);
    if (!tag) {
        return null;
    }
...
function getTagAtOffset(
    ...
    return getLatestOpeningTag(hamberDoc, offset);
}
...
const tagPossibilities: Array<{ tag: HamberTag | ':else'; values: string[] }> = [
