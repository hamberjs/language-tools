...
export async function getSelectionRange(hamberDoc: HamberDocument, position: Position) {
    const { script, style, moduleScript } = hamberDoc;
    const {
        ast: { html }
    } = await hamberDoc.getCompiled();
