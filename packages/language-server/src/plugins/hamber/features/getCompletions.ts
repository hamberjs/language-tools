...
import { HamberTag, documentation, getLatestOpeningTag } from './HamberTags';
...
export function getCompletions(
    ...
    const lastCharactersBeforePosition = hamberDoc
        .getText()
    ...
    /**
     * Get completions for special hamber tags within moustache tags.
     */
    function getTagCompletionsWithinMoustache() {
        ...
        return getCompletionsWithRegardToTriggerCharacter(triggerCharacter, hamberDoc, offset);
    }
...
function showCompletionWithRegardsToOpenedTags(
    ...
    switch (getLatestOpeningTag(hamberDoc, offset)) {
        case 'each':
            return on.eachOpen;
        case 'if':
            return on.ifOpen;
...
function createCompletionItems(
    items: Array<{ label: string; tag: HamberTag; insertText?: string }>
): CompletionList {
