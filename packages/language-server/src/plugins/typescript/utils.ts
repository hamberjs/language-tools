...
export function isHamberFilePath(filePath: string) {
    return filePath.endsWith('.hamber');
}

export function isVirtualHamberFilePath(filePath: string) {
    return filePath.endsWith('.hamber.ts');
}

export function toRealHamberFilePath(filePath: string) {
    return filePath.slice(0, -'.ts'.length);
}

export function toVirtualHamberFilePath(filePath: string) {
    return filePath.endsWith('.ts') ? filePath : filePath + '.ts';
}

export function ensureRealHamberFilePath(filePath: string) {
...
export function convertToLocationRange(snapshot: DocumentSnapshot, textSpan: ts.TextSpan): Range {
    ...
    // Some definition like the hamber component class definition don't exist in the original, so we map to 0,1
...
export function isInScript(position: Position, snapshot: HamberDocumentSnapshot | Document) {
...
export function changeHamberComponentName(name: string) {
    return name.replace(/(\w+)__HamberComponent_/, '$1');
}
