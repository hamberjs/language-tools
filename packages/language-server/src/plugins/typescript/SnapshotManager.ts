...
export class GlobalSnapshotsManager {
    ...
    updateTsOrJsFile(
        ...
        if (changes) {
            ...
            const newSnapshot = DocumentSnapshot.fromNonHamberFilePath(fileName, this.tsSystem);
        ...
        if (date.getTime() - this.lastLogged.getTime() > 60_000) {
            ...
            Logger.log(
                'SnapshotManager File Statistics:\n' +
                    ...
                    `Hamber files: ${
                        allFiles.filter((name) => name.endsWith('.hamber')).length
                    }\n` +
                    `From node_modules: ${
...
export const ignoredBuildDirectories = ['__ramber__', '.hamber-kit'];
