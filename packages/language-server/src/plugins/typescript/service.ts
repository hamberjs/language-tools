...
import { DocumentSnapshot, HamberSnapshotOptions } from './DocumentSnapshot';
import { createHamberModuleLoader } from './module-loader';
import {
    GlobalSnapshotsManager,
    ignoredBuildDirectories,
    SnapshotManager
} from './SnapshotManager';
import { ensureRealHamberFilePath, findTsConfigPath, hasTsExtensions, isSubPath } from './utils';
...
async function createLanguageService(
    ...
    const hamberModuleLoader = createHamberModuleLoader(getSnapshot, compilerOptions, tsSystem);

    let hamberTsPath: string;
    try {
        // For when hamber2tsx/hamber-check is part of node_modules, for example VS Code extension
        hamberTsPath = dirname(require.resolve(docContext.ambientTypesSource));
    } catch (e) {
        // Fall back to dirname
        hamberTsPath = __dirname;
    }
    const hamberTsxFiles = [
        './hamber-shims.d.ts',
        './hamber-jsx.d.ts',
        './hamber-native-jsx.d.ts'
    ].map((f) => tsSystem.resolvePath(resolve(hamberTsPath, f)));
    ...
    const host: ts.LanguageServiceHost = {
        ...
        fileExists: hamberModuleLoader.fileExists,
        readFile: hamberModuleLoader.readFile,
        resolveModuleNames: hamberModuleLoader.resolveModuleNames,
        readDirectory: hamberModuleLoader.readDirectory,
    ...
    function deleteSnapshot(filePath: string): void {
        hamberModuleLoader.deleteFromModuleCache(filePath);
    ...
    function updateSnapshotFromDocument(document: Document): DocumentSnapshot {
        ...
        if (!prevSnapshot) {
            hamberModuleLoader.deleteUnresolvedResolutionsFromCache(filePath);
        }
    ...
    function getSnapshot(fileName: string): DocumentSnapshot {
        fileName = ensureRealHamberFilePath(fileName);
        ...
        hamberModuleLoader.deleteUnresolvedResolutionsFromCache(fileName);
    ...
    function getParsedConfig() {
        ...
        if (!docContext.useNewTransformation) {
            // these are needed to handle the results of hamber2tsx preprocessing:
            ...
            [
                {
                    ...
                    // If Deferred exists, this means that all Hamber files are included
        ...
        // detect which JSX namespace to use (hamber | hamberNative) if not specified or not compatible
        if (!compilerOptions.jsxFactory || !compilerOptions.jsxFactory.startsWith('hamber')) {
            if (!docContext.useNewTransformation) {
                //default to regular hamber, this causes the usage of the "hamber.JSX" namespace
                compilerOptions.jsxFactory = 'hamber.createElement';
            }

            //override if we detect hamber-native
            if (workspacePath) {
                try {
                    const hamberNativePkgInfo = getPackageInfo('hamber-native', workspacePath);
                    if (hamberNativePkgInfo.path) {
                        if (docContext.useNewTransformation) {
                            // For backwards compatibility
                            parsedConfig.raw.hamberOptions = parsedConfig.raw.hamberOptions || {};
                            parsedConfig.raw.hamberOptions.namespace = 'hamberNative.JSX';
                        } else {
                            compilerOptions.jsxFactory = 'hamberNative.createElement';
                        }
                    }
                } catch (e) {
                    //we stay regular hamber
                }
            }
        }
