...
interface LSAndTSDocResolverOptions {
    ...
    /**
     * True, if used in the context of hamber-check
     */
    isHamberCheck?: boolean;

    /**
     * This should only be set via hamber-check. Makes sure all documents are resolved to that tsconfig. Has to be absolute.
     */
    tsconfigPath?: string;
...
export class LSAndTSDocResolver {
    ...
    /**
     * Create a hamber document -> should only be invoked with hamber files.
     */
    ...
    private get lsDocumentContext(): LanguageServiceDocumentContext {
        return {
            ambientTypesSource: this.options?.isHamberCheck ? 'hamber-check' : 'hamber2tsx',
            createDocument: this.createDocument,
            useNewTransformation: this.configManager.getConfig().hamber.useNewTransformation,
            transformOnTemplateError: !this.options?.isHamberCheck,
     ...
     */
    async getSnapshot(document: Document): Promise<HamberDocumentSnapshot>;
    ...
    async deleteSnapshot(filePath: string) {
        ...
        if (this.docManager.get(uri)) {
            ...
            // also this may not be a Hamber file
