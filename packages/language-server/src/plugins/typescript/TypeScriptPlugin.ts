...
import { isAttributeName, isAttributeShorthand, isEventHandler } from './hamber-ast-utils';
import {
    ...
    async getDocumentSymbols(
        ...
        for (let symbol of symbols.slice(1)) {
            ...
            if (
                ...
                symbol.name.startsWith('__hamberts_')
            ) {
                continue;
            }
            ...
            if (
                ...
                const node = tsDoc.hamberNodeAt(symbol.location.range.start);
                if (
    ...
    async getDefinitions(document: Document, position: Position): Promise<DefinitionLink[]> {
        ...
        const result = await Promise.all(
            defs.definitions.map(async (def) => {
                if (def.fileName.endsWith('hamber-shims.d.ts')) {
                    return;
                }
    ...
    async updateImports(fileRename: FileRename): Promise<WorkspaceEdit | null> {
        if (
            !(
                ...
                this.configManager.enabled('hamber.rename.enable')
            )
        ) {
            return null;
        }
        ...
        for (const { fileName, changeType } of onWatchFileChangesParas) {
            ...
            const canSafelyIgnore =
                ...
                ignoredBuildDirectories.some((dir) => {
                    ...
                    return (
                        // Files in .hamber-kit/types should always come through
                        index > 0 && (dir !== '.hamber-kit' || dirPathParts[index + 1] !== 'types')
                    );
                });
            if (canSafelyIgnore) {
                continue;
            ...
            if (scriptKind === ts.ScriptKind.Unknown) {
                // We don't deal with hamber files here
                continue;
            }
