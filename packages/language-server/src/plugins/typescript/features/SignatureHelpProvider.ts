...
export class SignatureHelpProviderImpl implements SignatureHelpProvider {
    ...
    async getSignatureHelp(
        ...
        if (
            !info ||
            info.items.some((signature) => this.isInHamber2tsxGeneratedFunction(signature))
        ) {
            return null;
        }
    ...
    private isInHamber2tsxGeneratedFunction(signatureHelpItem: ts.SignatureHelpItem) {
        return signatureHelpItem.prefixDisplayParts.some((part) =>
            part.text.includes('__hamberts')
        );
    }
}
