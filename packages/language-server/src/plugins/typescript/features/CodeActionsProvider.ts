...
import { changeHamberComponentName, convertRange } from '../utils';
...
export class CodeActionsProviderImpl implements CodeActionsProvider {
    ...
    private checkRemoveImportCodeActionRange(
        ...
        // Handle hamber2tsx wrong import mapping:
        // The character after the last import maps to the start of the script
        // TODO find a way to fix this in hamber2tsx and then remove this
        if (
            ...
            // line break generated by hamber2tsx,
                ...
                return TextDocumentEdit.create(
                    ...
                    change.textChanges
                        .map((edit) => {
                            if (
                                fix.fixName === 'import' &&
                                snapshot instanceof HamberDocumentSnapshot
                            ) {
    ...
    private getComponentImportQuickFix(
            ...
            (node): node is ts.JsxOpeningLikeElement | ts.JsxClosingElement | ts.Identifier =>
                this.configManager.getConfig().hamber.useNewTransformation
                    ? ts.isCallExpression(node.parent) &&
                      ...
                      node.parent.expression.text === '__hamberts_2_ensureComponent' &&
        ...
        const suffixedName = name + '__HamberComponent_';
        const errorPreventingUserPreferences =
            this.completionProvider.fixUserPreferencesForHamberComponentImport(userPreferences);
        ...
        const toFix = (c: ts.CompletionEntry) =>
            lang
                ...
                ?.codeActions?.map((a) => ({
                    ...a,
                    description: changeHamberComponentName(a.description),
                    fixName: 'import'
                })) ?? [];
    ...
    private async getApplicableRefactors(
        ...
        // Reason: Extracting that would lead to hamber2tsx' transformed store representation
        ...
        return (
            this.applicableRefactorsToCodeActions(applicableRefactors, document, range, textRange)
                ...
                // which is outside of the render function, which is hamber2tsx-specific and unmapped,
    ...
    /**
     * Some refactorings place the new code at the end of hamber2tsx' render function,
    ...
    /**
     * hamber2tsx removes export in instance script
     */
