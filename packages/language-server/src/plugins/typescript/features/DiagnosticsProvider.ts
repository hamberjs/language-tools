...
function isNoFalsePositive(
    ...
    return (diagnostic: Diagnostic) => {
        if (
            ...
            const node = tsDoc.hamberNodeAt(diagnostic.range.start);
...
function enhanceIfNecessary(diagnostic: Diagnostic): Diagnostic {
    if (
        ...
        (diagnostic.code === DiagnosticCode.TYPE_X_NOT_ASSIGNABLE_TO_TYPE_Y &&
            diagnostic.message.includes('ConstructorOfATypedHamberComponent'))
    ) {
        return {
            ...diagnostic,
            message:
                ...
                '- Type definitions are missing for this Hamber Component. ' +
                'If you are using Hamber 3.31+, use HamberComponentTyped to add a definition:\n' +
                '  import type { HamberComponentTyped } from "hamber";\n' +
                '  class ComponentName extends HamberComponentTyped<{propertyName: string;}> {}'
        };
    }
    ...
    if (diagnostic.code === DiagnosticCode.JSX_ELEMENT_DOES_NOT_SUPPORT_ATTRIBUTES) {
        return {
            ...
            message:
                ...
                'type definitions are missing for this Hamber Component or element cannot be used as such.\n\n' +
                'Underlying error:\n' +
                diagnostic.message
        };
    }
 ...
 * a reactive statement, and those actually have side effects in Hamber (hinting deps).
 *
 *     $: x, update()
 *
