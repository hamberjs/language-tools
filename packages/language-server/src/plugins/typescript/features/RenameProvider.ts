...
import { DocumentSnapshot, HamberDocumentSnapshot } from '../DocumentSnapshot';
import { convertRange } from '../utils';
import { LSAndTSDocResolver } from '../LSAndTSDocResolver';
import ts from 'typescript';
import {
    isComponentAtPosition,
    isAfterHamber2TsxPropsReturn,
...
import { isAttributeName, isEventHandler } from '../hamber-ast-utils';
...
export class RenameProviderImpl implements RenameProvider {
    ...
    private getRenameInfo(
        ...
        | null {
        // Don't allow renames in error-state, because then there is no generated hamber2tsx-code
        ...
        const hamberNode = tsDoc.hamberNodeAt(originalPosition);
        if (
            this.configManager.getConfig().hamber.useNewTransformation &&
            (isInHTMLTagRange(doc.html, doc.offsetAt(originalPosition)) ||
                isAttributeName(hamberNode, 'Element') ||
                isEventHandler(hamberNode, 'Element'))
        ) {
            return null;
        }
    ...
    private async getAdditionLocationsForRenameOfPropInsideComponentWithProp(
        ...
        // This requires hamber2tsx to have the properties written down like `return props: {bla: bla}`.
        ...
        const replacementsForProp = (
            ...
            (rename) =>
                ...
                this.isInHamber2TsxPropLine(tsDoc, rename)
        );
     ...
     * we need to handle the rename of the prop of A ourselves.
     * Reason: the rename will rename the prop in the computed hamber2tsx code,
    ...
    private async getAdditionalLocationsForRenameOfPropInsideOtherComponent(
        ...
        const doc = <HamberDocumentSnapshot>snapshots.get(updatePropLocation.fileName);
    ...
    private matchGeneratedExportLet(
        ...
        const regex = new RegExp(
            // no 'export let', only 'let', because that's what it's translated to in hamber2tsx
            `\\s+let\\s+(${snapshot
                .getFullText()
                .substring(
    ...
    private findLocationWhichWantsToUpdatePropName(
        ...
        return convertedRenameLocations.find((loc) => {
            ...
            // Props are in hamber snapshots only
            if (!(snapshot instanceof HamberDocumentSnapshot)) {
                return false;
            }

            return this.isInHamber2TsxPropLine(snapshot, loc);
        });
    }

    // --------> hamber2tsx?
    private isInHamber2TsxPropLine(snapshot: HamberDocumentSnapshot, loc: ts.RenameLocation) {
        return isAfterHamber2TsxPropsReturn(snapshot.getFullText(), loc.textSpan.start);
    }

    /**
     * The rename locations the ts language services hands back are relative to the
     * hamber2tsx generated code -> map it back to the original document positions.
    ...
    private filterWrongRenameLocations(
        ...
        return filterAsync(mappedLocations, async (loc) => {
            ...
            // When the user renames a Hamber component, ts will also want to rename
            // `__hamberts_1_instanceOf(TheComponentToRename)` or
            // `__hamberts_1_ensureType(TheComponentToRename,..`. Prevent that.
            // Additionally, we cannot rename the hidden variable containing the store value
            return (
                notPrecededBy('__hamberts_1_instanceOf(') &&
                notPrecededBy('__hamberts_1_ensureType(') && // no longer necessary for new transformation
                notPrecededBy('= __hamberts_1_store_get(')
            );
    ...
    private mapRangeToOriginal(snapshot: DocumentSnapshot, textSpan: ts.TextSpan): Range {
        // We need to work around a current hamber2tsx limitation: Replacements and
        ...
        // 2. the generated hamber2tsx code will not modify variable names, so we know
    ...
    private checkShortHandBindingOrSlotLetLocation(
        ...
        return renameLocations.map((location) => {
            ...
            if (this.configManager.getConfig().hamber.useNewTransformation) {
