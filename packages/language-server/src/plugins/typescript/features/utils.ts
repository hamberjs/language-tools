...
/**
 * If the given original position is within a Hamber starting tag,
...
/**
 * Checks if this is a text span that is inside hamber2tsx-generated code
...
export function isStoreVariableIn$storeDeclaration(text: string, varStart: number) {
    return (
        text.lastIndexOf('__hamberts_1_store_get(', varStart) ===
        varStart - '__hamberts_1_store_get('.length
    );
}
...
export function is$storeVariableIn$storeDeclaration(text: string, varStart: number) {
    return /^\$\w+ = __hamberts_1_store_get/.test(text.substring(varStart));
}
...
export function isAfterHamber2TsxPropsReturn(text: string, end: number) {
    const textBeforeProp = text.substring(0, end);
    // This is how hamber2tsx writes out the props
...
export const isReactiveStatement = nodeAndParentsSatisfyRespectivePredicates<ts.LabeledStatement>(
    ...
    or(
        // function render() {
        //     $: x2 = __hamberts_1_invalidate(() => x * x)
        // }
...
export function isKitTypePath(path?: string): boolean {
    return !!path?.includes('.hamber-kit/types');
}
