...
import { HamberDocumentSnapshot } from '../DocumentSnapshot';
import { LSAndTSDocResolver } from '../LSAndTSDocResolver';
import { getMarkdownDocumentation } from '../previewer';
import {
    changeHamberComponentName,
    convertRange,
...
export interface CompletionEntryWithIdentifier extends ts.CompletionEntry, TextDocumentIdentifier {
    position: Position;
    __is_hamberkit$typeImport?: boolean;
}
...
export class CompletionsProviderImpl implements CompletionsProvider<CompletionEntryWithIdentifier> {
    ...
    async getCompletions(
        ...
        const hamberNode = tsDoc.hamberNodeAt(originalOffset);
        if (
            // Cursor is somewhere in regular HTML text
            (hamberNode?.type === 'Text' &&
                ['Element', 'InlineComponent', 'Fragment', 'SlotTemplate'].includes(
                    hamberNode.parent?.type as any
                )) ||
        ...
        if (
            completions.length > 500 &&
            hamberNode?.type === 'InlineComponent' &&
            ['  ', ' >', ' /'].includes(
        ...
        if (basename(filePath).startsWith('+')) {
            ...
            for (const $typeImport of $typeImports.values()) {
                // resolve path from filePath to hamber-kit/types
                ...
                if (relativeFileName) {
                    ...
                    const modifiedSource =
                        $typeImport.data.source.split('.hamber-kit/types')[0] +
    ...
    private toCompletionItem(
        snapshot: HamberDocumentSnapshot,
        ...
        const { label, insertText, isHamberComp, replacementSpan } = completionLabelAndInsert;
        // TS may suggest another Hamber component even if there already exists an import
        // with the same name, because under the hood every Hamber component is postfixed
        // with `__HamberComponent`. In this case, filter out this completion by returning null.
        if (isHamberComp && existingImports.has(label)) {
            return null;
        }
        ...
        return {
            ...
            // Make sure hamber component takes precedence
            sortText: isHamberComp ? '-1' : comp.sortText,
            preselect: isHamberComp ? true : comp.isRecommended,
            textEdit,
    ...
    private getCompletionLabelAndInsert(
        ...
        const isHamberComp = this.isHamberComponentImport(name);
        if (isHamberComp) {
            name = changeHamberComponentName(name);

            if (this.isExistingHamberComponentImport(snapshot, name, comp.source)) {
                return null;
            }
        }
        ...
        if (comp.replacementSpan) {
            return {
                label: name,
                isHamberComp,
                insertText: insertText ? changeHamberComponentName(insertText) : undefined,
    ...
    private isExistingHamberComponentImport(
        snapshot: HamberDocumentSnapshot,
        name: string,
        source?: string
    ): boolean {
        const importStatement = new RegExp(`import ${name} from ["'\`][\\s\\S]+\\.hamber["'\`]`);
     ...
     * 'js' and there's an unknown file extension - which is the case for `.hamber`. Therefore
     * rewrite the importModuleSpecifierEnding for this case to silence the error.
     */
    fixUserPreferencesForHamberComponentImport(
    ...
    async resolveCompletion(
        ...
        const is$typeImport = !!comp.__is_hamberkit$typeImport;

        const errorPreventingUserPreferences = comp.source?.endsWith('.hamber')
            ? this.fixUserPreferencesForHamberComponentImport(userPreferences)
            : userPreferences;
    ...
    private getCompletionDocument(compDetail: ts.CompletionEntryDetails, is$typeImport: boolean) {
        ...
        let detail: string = changeHamberComponentName(ts.displayPartsToString(displayParts));
    ...
    codeActionChangeToTextEdit(
        ...
        if (!scriptTagInfo) {
            ...
            const lang = this.configManager.getConfig().hamber.defaultScriptLanguage;
    ...
    private mapRangeForNewImport(snapshot: HamberDocumentSnapshot, virtualRange: Range) {
    ...
    private isHamberComponentImport(className: string) {
        return className.endsWith('__HamberComponent_');
    }
    ...
    private changeComponentImport(
        ...
        const changedName = changeHamberComponentName(importText);
        if (importText !== changedName || !actionTriggeredInScript) {
            // For some reason, TS sometimes adds the `type` modifier. Remove it
            // in case of Hamber component imports or if import triggered from markup.
    ...
    private jsxTransformationPropStringLiteralCompletion(
        ...
        componentInfo: ComponentInfoProvider | null,
        position: number,
        tsDoc: HamberDocumentSnapshot
    ) {
        if (!componentInfo || this.configManager.getConfig().hamber.useNewTransformation) {
            return null;
        }
...
// Type definitions from hamber-shims.d.ts that shouldn't appear in completion suggestions
// because they are meant to be used "behind the scenes"
const hamber2tsxTypes = new Set([
    'Hamber2TsxComponent',
    'Hamber2TsxComponentConstructorParameters',
    'HamberComponentConstructor',
    'HamberActionReturnType',
    'HamberTransitionConfig',
    'HamberTransitionReturnType',
    'HamberAnimationReturnType',
    'HamberWithOptionalProps',
    'HamberAllProps',
    'HamberPropsAnyFallback',
    'HamberSlotsAnyFallback',
    'HamberRestProps',
    'HamberSlots',
    'HamberStore'
]);
...
function isValidCompletion(
    ...
    const isNoHamber2tsxCompletion = (value: ts.CompletionEntry) =>
        value.kindModifiers !== 'declare' ||
        (!value.name.startsWith('__hamberts_') && !hamber2tsxTypes.has(value.name));
    ...
    if (!isCompletionInHTMLStartTag) {
        return isNoHamber2tsxCompletion;
    }
    ...
    return (value) =>
        ...
        isNoHamber2tsxCompletion(value) &&
